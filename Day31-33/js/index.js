...
function getItem(items,k) {
    for(let i=0;i<sourceData.length;i++) {
        //这一步很重要，因为没有的话就会出现华东华北华南华东华北华南华东华北华南全选
...
item1 = getItem(item1,'region');// ["华东", "华北", "华南"]
item2 = getItem(item2,'product');// ["手机", "笔记本", "智能音箱"]
//console.log(item1);
...
function choose(el,id) {
    el.addEventListener('click',function(e) {
        ...
        switch(target.id) {
             // 当id存在的情况，也就是全选时，点击将所有checkbox都选上,这里为了对应createCheckbox(items,el,id)，给全选创建了一个id
             case id:
...
function getData() {
    ...
    list = [];
    for(let i=0;i<ipts.length;i++) {
        //如果不是全选的checkbox，就把它的值添加到arr中
        if(!ipts[i].id) {
            arr.push(ipts[i].value);
        }
    }
    //例如arr中只有华南，则list最后会包含sourceData中的所有包含华南的数据
    for(let i=0;i<sourceData.length;i++) {
...
//初始化页面
createCheckbox(item1,regionCheck,'regall');
createCheckbox(item2,productCheck,'proall');
getData();
createTable();
mergeCell(1,0);
